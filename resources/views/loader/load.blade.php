                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <form id="file_form" class="form-horizontal" method="POST"
                              action="{{ route('loader/loadDiagnosticFile') }}" enctype="multipart/form-data">
                            {{ csrf_field() }}
                            ...
                            <div class="form-group{{ $errors->has('diagnostic_file') ? ' has-error' : '' }}">
                                ...
                                <div class="col-md-6">
                                    <input id="diagnostic_file" type="file" class="form-control" name="diagnostic_file"
                                           value="{{ old('diagnostic_file') }}" required autofocus accept=".xlsx">
...
<script>
    ...
    function insertErrorMsg(inputElement, msg) {
        var inputElementName = inputElement.attr('name'),
            msgList = {
                'diagnostic_file': 'Необходимо выбрать файл для загрузки',
        ...
        if (inputElement.siblings('div.error').length < 1) {
            inputElement.after('<div class="error" style="color: red">' + msg + '</div>')
        }
    }
